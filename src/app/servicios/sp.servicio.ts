...
export class SPServicio {
    ...
    public ObtenerConfiguracionConPost() {
        ...
        const configuracionSharepoint = pnp.sp.configure({
            headers: {
                ...
                'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6ImllX3FXQ1hoWHh0MXpJRXN1NGM3YWNRVkduNCIsImtpZCI6ImllX3FXQ1hoWHh0MXpJRXN1NGM3YWNRVkduNCJ9..wdAReVIopR9KKXUwO2_0BbOi9F-qyJgvg2qJfeUF3mY5VjOtoFPSjV9M8PdpsV0F76cFRzx0zBslt2-HLB4bC9_8L57XtpT3Iu8Ho_Hl8mll3ERfxoi5EVnnltdzUPC-iO8URugAtN14POMkIClcvdi_tQb-JanJ42PTYDXGCEBx8MgGNf_BiIN-cAesARGy8T5jSXwXn8TTn0z_kkQCB2RrcCL8d-VH_xqF5Jgsm2_i_BZmxdZmPrz8bRPUXbuMo36BkxFHtqsTT5mhvNaHkD0zoSVd8CxmUvy7L60uVMKn3OTyQlDN9HdeDetSvGGf36S045WWkap9qrjrTptxXA'
            }
        }, environment.urlWeb);
    ...
    async agregarAdjuntoActividad(nombreLista: string, nombreArchivo: string, archivo: File): Promise<any> {
        let elemento = await this.ObtenerConfiguracionConPost().web.folders.getByName(nombreLista).files.add(nombreArchivo,archivo);
        return elemento;        
    }
    ...
    actualizarPropiedadesAdjuntoActividad(nombreLista,idDoc, IdActividad, IdRegistro){
        ...
        // })
        let elemeto = this.ObtenerConfiguracionConPost().web.lists.getByTitle(nombreLista).items.getById(idDoc).update({
            IdActividad: IdActividad,
            IdRegistro: IdRegistro,
        });
        return elemeto;
